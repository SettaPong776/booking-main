...
function initEditProfile(prefix) {
  ...
  const telegram_login = document.getElementById('telegram_login');
  if (telegram_login) {
    const script = document.createElement('script');
    script.src = 'https://telegram.org/js/telegram-widget.js?22';
    script.setAttribute('data-telegram-login', telegram_login.getAttribute('data-username'));
    script.setAttribute('data-size', 'medium');
    script.setAttribute('data-onauth', 'onTelegramGet(user)');
    script.setAttribute('data-request-access', 'write');
    telegram_login.appendChild(script);
  }
}

function onTelegramGet(user) {
  $E('register_telegram_id').value = user.id;
}
...
function onTelegramAuth(user) {
  ...
  for (var prop in user) {
    q.push(prop + '=' + encodeURIComponent(user[prop]));
  }
  send(WEB_URL + "index.php/index/model/telegramlogin/chklogin", q.join("&"), socialLoginSubmit);
}
...
function socialLoginSubmit(xhr) {
  var ds = xhr.responseText.toJSON();
  if (ds) {
    if (ds.alert) {
      alert(ds.alert);
