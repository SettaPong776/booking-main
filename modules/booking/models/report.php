...
class Model extends \Kotchasan\Model
{
    ...
    public static function toDataTable($params)
    {
        ...
        $select = ['V.id', 'V.topic', 'R.name', 'R.color'];
            ...
            if ($reportApprove != 0) {
                ...
                if (preg_match_all('/,?([0-9]+),?/', $request->post('id')->toString(), $match)) {
                    if ($action === 'delete') {
                        ...
                        if (!empty($ids)) {
                            // ลบ
                            $this->db()->delete($this->getTableName('reservation'), ['id', $match[1]], 0);
                            $this->db()->delete($this->getTableName('reservation_data'), ['reservation_id', $match[1]], 0);
                            // log
