...
class Model extends \Kotchasan\Model
{
    ...
    public static function toDataTable($params)
    {
        ...
        if ($params['room_id'] > 0) {
            $where[] = ['V.room_id', $params['room_id']];
        }
        if ($params['status'] > -1) {
        ...
        $select = ['V.topic', 'V.id', 'V.room_id', 'V.approve', 'V.closed'];
        ...
        $select = array_merge($select, ['V.begin', 'V.end', 'V.status', 'V.reason', $sql, 'R.color']);
    ...
    public function action(Request $request)
    {
        ...
        if ($request->initSession() && $request->isReferer()) {
            ...
            if ($action === 'cancel' && $login) {
                ...
                if ($search && $login['id'] == $search->member_id && in_array($search->status, self::$cfg->booking_delete)) {
                    // ลบ
                    $db->delete($reservation_table, $search->id);
                    // Log
                    \Index\Log\Model::add(0, 'booking', 'Delete', '{LNG_Delete} ID : '.$search->id, $login['id']);
                    // ลบเรียบร้อย
    ...
    public function bookDetail($id)
    {
        $query = $this->db()->createQuery()
            ...
            ->where(['V.id', $id])
            ->toArray();
        $select = ['V.*', 'R.name', 'U.name contact', 'U.phone', 'R.color'];
        $n = 1;
