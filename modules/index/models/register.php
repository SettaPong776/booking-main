            ...
            if ((!empty(self::$cfg->user_register) && self::$cfg->demo_mode == false) || $isAdmin) {
                try {
                    ...
                    foreach (Language::get('LOGIN_FIELDS') as $field => $label) {
                        ...
                        if ($request->post('register_'.$k)->exists()) {
                            ...
                            if (empty($value)) {
                                ...
                                // อีเมลไม่ถูกต้อง
                                $ret['ret_register_'.$k] = Language::replace('Invalid :name', [':name' => $label]);
                            } else {
                    ...
                    foreach ($checking as $k => $value) {
                        $search = $db->first($table_user, [$k, $value]);
                        if ($search) {
                            $ret['ret_register_'.$k] = Language::replace('This :name already exist', [':name' => Language::get('LOGIN_FIELDS', '', $k)]);
                        } else {
                            $save[$k] = $value;
                        }
                    }
                    ...
                    if (empty($ret)) {
                        ...
                        if ($isAdmin) {
                            ...
                            if ($err != '') {
                                ...
                                // no activate
                                $ret['alert'] = Language::replace('Register successfully, We have sent complete registration information to :email', [':email' => $save['username']]);
                            }
                        } else {
                            // คืนค่า
    ...
    public static function execute($model, $save, $permission = null, $user_meta = [])
    {
        ...
        if ($permission === null) {
            if (self::$cfg->demo_mode) {
                ...
                $permission = array_diff($permission, ['can_config', 'can_view_usage_history']);
            } else {
        ...
        if (!empty($user_meta)) {
            ...
            foreach ($user_meta as $key => $category) {
                $db->delete($table_user_meta, [['member_id', $save['id']], ['name', $key]], 0);
